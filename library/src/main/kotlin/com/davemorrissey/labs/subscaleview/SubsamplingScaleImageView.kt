...
open class SubsamplingScaleImageView @JvmOverloads constructor(
	...
	companion object {
		...
		private val ZOOM_IN_THRESHOLD =
			0.05f   // if the user zooms in a bit, do not allow rotating the image with the given gesture anymore
	}
	...
	fun setImage(path: String) {
		...
		scope?.launch {
				...
				if (!uriFile.exists()) {
					try {
						newPath = withContext(Dispatchers.IO) {
							URLDecoder.decode(newPath, "UTF-8")
						}
			...
			if (!context.packageName.startsWith("com.davemorrissey") && !context.packageName.startsWith(
			...
			val task = TilesInitTask(this@SubsamplingScaleImageView, context, regionDecoderFactory, uri!!)
			task.executeTask()
		}
	...
	private fun onTouchEventInternal(event: MotionEvent): Boolean {
		...
		when (event.action) {
			MotionEvent.ACTION_DOWN, MotionEvent.ACTION_POINTER_DOWN or 0x0000, MotionEvent.ACTION_POINTER_DOWN or 0x0100 -> {
			...
			MotionEvent.ACTION_UP, MotionEvent.ACTION_POINTER_UP, MotionEvent.ACTION_POINTER_UP or 0x0100 -> {
				if (isQuickScaling) {
					isQuickScaling = false
					if (quickScaleMoved) {
						animateToBounds()
	...
	override fun onDraw(canvas: Canvas) {
					...
					for (tile in value) {
						...
						if (!tile.loading && tile.bitmap != null) {
							...
							objectMatrix!!.postRotate(
								Math.toDegrees(imageRotation).toFloat(),
								width / 2f,
								height / 2f
							)
		...
		if (debug) {
			canvas.drawText(
				...
				} (${String.format(Locale.ENGLISH, "%.2f", getFullScale())} - ${
					String.format(
						Locale.ENGLISH,
						"%.2f",
						maxScale
	...
	fun animateToBounds(forceInstantRefresh: Boolean = false) {
		...
		if (scale >= fullScale) {
			...
			AnimationBuilder(center, rightAngle).apply {
				duration =
					if (isZoomedIn || forceInstantRefresh) INSTANT_ANIMATION_DURATION else ANIMATION_DURATION
				start()
			}
		} else {
	...
	private class TilesInitTask(
			...
			withContext(Dispatchers.IO) {
				try {
					...
					if (context != null && decoderFactory != null && view != null) {
						...
						xyo = intArrayOf(sWidth, sHeight, exifOrientation)
					}
				} catch (e: Exception) {
					exception = e
				}
		...
		private fun taskDone(xyo: IntArray?) {
			val view = viewRef.get()
			if (view != null) {
	...
	private class TileLoadTask(
		...
		val decoderRef = WeakReference(decoder)
		val tileRef = WeakReference(tile)
		...
		private fun taskDone(bitmap: Bitmap?) {
	...
	private class BitmapLoadTask(
		...
		suspend fun executeTask() {
			...
			taskDone(orientation)
		}

		private fun taskDone(orientation: Int?) {
			val subsamplingScaleImageView = viewRef.get()
